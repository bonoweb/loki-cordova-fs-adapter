                ...
                this._getFile(dbname, function (fileEntry) {
                    fileEntry.createWriter(function (fileWriter) {
                        ...
                        fileWriter.onAbort = function(){
                            console.log(TAG,"abort writing file!");
                        };
                        fileWriter.onwrite = function () {
                            if (fileWriter.length === 0) {
                                fileWriter.write(blob);
                                //console.log(TAG,"writed db");
                                //callback();
                            }
                            else{
                                callback();
                ...
                this._getFile(dbname, function (fileEntry) {
                    fileEntry.file(function (file) {
                        ...
                        reader.onerror = function (event){
                            console.warn(TAG, "error reading file - " + event.toString());
                        }
                        reader.onload = function (event) {
                            var contents = event.target.result;
                            if (contents.length === 0) {
                                console.warn(TAG, "couldn't find database:"+dbname);
                                //now, re-trying
                                setTimeout(function(){
                                    ...
                                    reader2.onload = function (event) {
                                        var contents = event.target.result;
                                        if (contents.length === 0) {
                                            console.warn(TAG, "couldn't find database again!");
                                            callback(null);
                                        }
                                        else {
                                            callback(contents);
